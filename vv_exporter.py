...
def get_all_links(page):
    ...
    links = [
        details_from_element(x)
        for x in page.soup.select(SELECTOR)
        if x.text.strip() not in BLACKLIST and len(x.select("a")) > 0
    ]
    return links
...
def extract_module_details(html, browser):
    ...
    details = [
        sanitize_detail(
            {"title": x.split("</b>")[0].strip(), "details": x.split("</b>")[1].strip()}
        )
    ...
    try:
        ...
        kurs_appointments = [extract_rooms_and_times_of_module(x) for x in kurse_pages][
            -1
        ]
...
def get_vv(browser, start_page, base_url):
    # anmeldung page
    anmeldung_page_link = (
        base_url + start_page.soup.select('li[title="Anmeldung"] a')[0].attrs["href"]
    )
